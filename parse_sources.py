...
for item in raw_data:
    try:
        title = item[1]
        if not title:
            # Fallback title if empty, use first part of URL or "Fuente Web"
            url = item[2][7][0]
            title = "Fuente Web: " + url[:30] + "..."
        
        # Extract URL - it seems to be at index 7 inside the 3rd element list, formatted as ["url"]
        url_list = item[2][7]
        url = url_list[0] if url_list else "N/A"
        
        cleaned_sources.append({
            "title": title,
            "author": "Fuente NotebookLM", # Generic since author isn't clearly separated
            "description": url # Use URL as description/link
        })
